...
Generated files are stored in the output/ directory when the system
is executed.
...
Before execution, the system validates the configured traffic ratios
...
This allows the generated normal traffic to represent multiple IoT
devices with different communication patterns.
...
Generates a high-volume sequence of MQTT publish packets targeting an
MQTT broker on port 1883.

DNS Exfiltration

Generates DNS queries containing encoded-looking data within generated
subdomains.

Mirai-like Scanning

Generates TCP SYN packets targeting randomly selected IP addresses and
commonly targeted ports such as:
...
These scenarios are designed to reproduce characteristic network
traffic patterns for cybersecurity research rather than to execute
actual malware.
...
This allows the generated traffic to contain different temporal
patterns between attack scenarios.
...
Using the same configuration and random seed allows the same synthetic
experiment to be reproduced.

Changing the random seed produces different random selections such as
traffic event ordering, ports, device selections and attack bursts.

PCAP and Dataset Generation

The generated traffic is written to a PCAP file using Scapy.

The PCAP is then processed using Zeek to extract network flow
information.
...
The extracted flows are matched against the generated traffic metadata
and automatically assigned a label.
...
The attack traffic was distributed equally between the four attack
scenarios:
...
This project was developed as part of a bachelor's thesis on synthetic
IoT network traffic generation and its application to cybersecurity
research.

The generated traffic and datasets are intended for research,
experimentation and educational use, including network analysis and
intrusion detection experiments.

The attack scenarios simulate characteristic patterns of malicious
network behavior without implementing or executing real malware or
performing attacks against real networks.
